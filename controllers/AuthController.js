...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  const otp = newUser.createOTP("signup");
...
exports.verifySignupOTP = catchAsync(async (req, res, next) => {
  ...
  const user = await User.findOne({ email }).select(
    "+otpCode +otpExpires +otpPurpose +otpAttempts",
  );

  if (!user) return next(new AppError("User not found", 400));

  if (user.otpPurpose !== "signup") {
    return next(new AppError("Invalid OTP request type", 400));
  }
...
exports.resendSignupOTP = catchAsync(async (req, res, next) => {
  ...
  const otp = user.createOTP("signup");
  await user.save({ validateBeforeSave: false });
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const otp = user.createOTP("reset");
  await user.save({ validateBeforeSave: false });
...
exports.resetPasswordWithOTP = catchAsync(async (req, res, next) => {
  ...
  if (!user.otpCode || !user.otpExpires) {
    return next(new AppError("No valid OTP found", 400));
  }
